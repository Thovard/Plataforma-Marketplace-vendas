    ...
    @foreach ($users as $user)
        <tr>
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                ...
                @else
                    <img src="{{url("image/favicon.ico") }}" alt="{{ $user->name }}" class="object-cover w-20">
                @endif
                {{ $user->name }}
            </td>
            ...
            <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                <a href="{{ route('comments.index', $user->id) }}" class="bg-red-200 rounded-full py-2 px-6">Anotação ({{ $user->comments->count()}})</a>
            </td>
        </tr>
    @endforeach
    </tbody>
